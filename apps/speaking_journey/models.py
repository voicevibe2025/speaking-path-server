...
class CoachAnalysisCache(models.Model):
    """
    Persistent cache for AI Coach analysis to survive server restarts.
    Stores the complete analysis JSON per user with expiration timestamp.
    """
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='coach_cache', primary_key=True)
    ...
    class Meta:
        ...
        verbose_name = _('Coach Analysis Cache')
        verbose_name_plural = _('Coach Analysis Caches')
        indexes = [
            models.Index(fields=['expires_at']),
        ]
    
    def __str__(self):
        return f"CoachCache for {self.user_id} - expires {self.expires_at}"
