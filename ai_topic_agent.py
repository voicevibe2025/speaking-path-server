...
def get_titles_from_topics() -> List[str]:
...
def build_suggestion_prompt(existing_titles: List[str], count: int) -> str:
    titles_str = "\n".join(f"- {t}" for t in existing_titles)
    return (
        "Suggest {count} new speaking practice topics (concise titles only).\n"
...
def build_topic_content_prompt(title: str) -> str:
    return (
...
def append_topics_to_file(new_topics: List[Dict[str, Any]]):
    content = read_topics_file()
    # Find the last closing bracket of TOPICS list
    m = re.search(r"\n\]\s*\Z", content, flags=re.S)
    if not m:
        raise RuntimeError("Could not locate closing bracket of TOPICS list in topics.py")
    insert_pos = m.start()
    ...
    for obj in new_topics:
        ...
        # indent every line by 4 spaces to match top-level list style
    ...
    # Find last non-whitespace char before insert_pos
    i = insert_pos - 1
    while i >= 0 and content[i] in (" ", "\t", "\r", "\n"):
        i -= 1
    if i >= 0 and content[i] == '}':
        # Prepend a comma and newline to separate previous item and new object
        insertion = ",\n" + insertion

    # Insert before the closing bracket, keeping existing blank lines
...
def main():
    ...
    print("Loading existing titles...")
    existing_titles = get_titles_from_topics()

    # 1) Suggest topics
    print(f"\nRequesting ~{args.suggest_count} topic suggestions from {provider}...")
    suggestion_prompt = build_suggestion_prompt(existing_titles, args.suggest_count)
    ...
    # 3) Generate full content for each selected and append
    new_topics: List[Dict[str, Any]] = []
    for title in sel:
        print(f"\nGenerating content for: {title}")
        content_prompt = build_topic_content_prompt(title)
    ...
    print(f"\nAbout to append {len(new_topics)} topic(s) to topics.py at the end of TOPICS list.")
    proceed = prompt_yes_no("Proceed with appending?")
    if not proceed:
        print("Aborted by user.")
        return

    append_topics_to_file(new_topics)
    print("Appended successfully to topics.py")
    ...
    if prompt_yes_no("Run 'manage.py seed_speaking_topics' using the project's venv?"):
        try:
            # Prefer venv Python if available
            py_cmd = resolve_venv_python() + ["manage.py", "seed_speaking_topics"]
